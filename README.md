...
**database/**
Chứa script SQL khởi tạo database. File `01_CreateDatabase.sql` tạo database `MenuNewsDb`, tạo ba bảng `Menus`, `News`, `MenuNews`, định nghĩa khóa chính, khóa ngoại, index và chèn dữ liệu mẫu. Script này phải được chạy trước khi scaffold EF Core.
...
Mở SSMS hoặc dùng sqlcmd để kết nối vào `localhost,1433` với tài khoản `sa` và mật khẩu `YourStrong@Passw0rd`. Mở file `database/01_CreateDatabase.sql` và thực thi toàn bộ script.

Nếu dùng sqlcmd từ terminal:

```bash
sqlcmd -S localhost,1433 -U sa -P "YourStrong@Passw0rd" -C -i database/01_CreateDatabase.sql
```
...
Tên collection lưu audit log. Mặc định là `audit_logs`.
...
info: MenuNews.Infrastructure.RabbitMq.NewsCreatedConsumerHostedService[0]
      RabbitMQ Consumer đang lắng nghe queue: news_created_queue
```
...
**Queue (hàng đợi)**: là nơi message được lưu tạm thời cho đến khi Consumer xử lý. Queue trong dự án có tên `news_created_queue`, được cấu hình là `durable: true` — nghĩa là message không bị mất khi RabbitMQ restart.
...
CreateNewsCommandHandler
    ...
    |-- [2] Gọi INewsEventPublisher.PublishNewsCreatedAsync(newsId, title)
        ...
        RabbitMqNewsEventPublisher
                ...
                |-- Declare queue "news_created_queue" nếu chưa có
                |-- Publish JSON message: { "NewsId": 3, "Title": "..." }
                |
                v
        RabbitMQ Broker (Queue: news_created_queue)
                |
                v
...
Đăng nhập bằng `guest` / `guest`. Vào tab **Queues** và tìm queue `news_created_queue`. Tại đây bạn có thể thấy số lượng message đã được publish, message đang chờ xử lý và thống kê throughput.
...
Hiện tại, audit log được ghi trong `CreateNewsCommandHandler` sau khi News được lưu thành công vào SQL Server. Dữ liệu ghi bao gồm action (`CREATE_NEWS`), user thực hiện và metadata gồm `Id` và `Title` của News vừa tạo.
...
option csharp_namespace = "MenuNews.Api.Grpc";
...
```bash
grpcurl -plaintext -d '{"page_number": 1, "page_size": 5}' localhost:5000 news.NewsGrpc/GetAllNews
```
...
CreateNewsCommandHandler.Handle()
    ...
    |      |-- publish JSON message lên queue "news_created_queue"
    |
    | [4] IAuditLogService.LogAsync("CREATE_NEWS", user, metadata)
    |      |-- Infrastructure: MongoAuditLogService
    |      |-- INSERT document vào MongoDB collection "audit_logs"
    |
    | [5] trả về NewsDto
    v
NewsController
...
RabbitMqNewsEventPublisher
    |
    | BasicPublish -> exchange: "" -> routingKey: "news_created_queue"
    v
RabbitMQ Broker
    |
    | message lưu trong queue "news_created_queue"
    v
...
CreateNewsCommandHandler (hoặc bất kỳ Handler nào cần log)
    |
    | IAuditLogService.LogAsync("CREATE_NEWS", "system", { Id, Title })
    v
MongoAuditLogService.LogAsync()
    ...
    v
MongoDB: database "MenuNewsAudit", collection "audit_logs"
    |
